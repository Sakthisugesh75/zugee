...
const localLeadsStore = [
  {
    ...
    name: "Sample Lead (Education)",
    email: "sample.education@example.com",
    phone: "+91 90000 00001",
    industry: "education",
    goal: "Fee collection reconciliation & automated WhatsApp receipts",
  ...
  },
  {
    ...
    name: "Sample Lead (Hospitality)",
    email: "sample.hospitality@example.com",
    phone: "+91 90000 00002",
    industry: "hospitality",
    goal: "Digital WhatsApp check-in and unified room folio",
    message: "Development sample record.",
    source_page: "pricing-estimator",
    status: "contacted",
  ...
  },
  {
    ...
    name: "Sample Lead (Logistics)",
    email: "sample.logistics@example.com",
    phone: "+91 90000 00003",
    industry: "logistics",
    goal: "Automated e-way bill generation and driver POD capture",
    message: "Development sample record.",
    source_page: "fit-quiz",
    status: "qualified",
...
/**
 * Add a newsletter subscriber. Duplicate emails are ignored so the response
 * never reveals whether an address was already subscribed.
 */
export async function insertSubscriber(email, sourcePage = "footer-newsletter") {
  if (shouldUseLocalStore()) {
    localSubscribers.add(email);
    return;
  }
  ...
  if (error) {
    throw new Error(`Supabase subscriber insert failed: ${error.message}`);
  }
}
...
export async function getLeadsList({ page = 1, limit = 25, status = "all", industry = "all", search = "" } = {}) {
  ...
  if (shouldUseLocalStore()) {
    ...
    if (safeSearch) {
      ...
      filtered = filtered.filter((l) =>
        [l.name, l.email, l.phone, l.reference_id].some((field) => field && field.toLowerCase().includes(q))
      );
    }
  ...
  if (safeSearch) {
    ...
    query = query.or(
      `name.ilike.${pattern},email.ilike.${pattern},phone.ilike.${pattern},reference_id.ilike.${pattern}`
    );
  }
